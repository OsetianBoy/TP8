...
routerCart.post('/:id/productos', async(req,res) => {
    ...
    if (Object.prototype.hasOwnProperty.call(body, 'productId')) {
        ...
        newProductoCarritoId 
            ? res.status(200).json({"success": "Producto agregado correctamente"})
            : res.status(400).json({"error": "Hubo algun problema. Es posible que el ID del Carrito o el ID del producto sea invalido?"})
        
    } else {
        res.status(400).json({"error": "la key la key debeser 'productId', favor de verificar."})
    }
...
routerCart.delete('/:id/productos/:id_prod', async(req, res) => {
    ...
    wasDeleted 
        ...
        : res.status(400).json({"error": "hubo algun problema"})
